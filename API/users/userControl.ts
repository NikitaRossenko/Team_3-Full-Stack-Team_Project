...
export const resetPassword = async (req: any, res: any) => {
  try {
    ...
    const randomHash = await bcrypt.hash(randomNumber.toString() , 10)
    const findUser = await UserModel.findOneAndUpdate({ userName, email, lastName } , {password:randomHash})
    if (!findUser) throw new Error("the detail not correct");


    console.log("random Hash" , randomHash)
    console.log("curPassword user " , findUser.password);
    res.status(201).send({ok:true , newPassword:randomNumber})
